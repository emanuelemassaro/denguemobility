...
void perform(long total_iterations, int current_rank, int world_size, int Iyear, int Inet)
{
    int V = 0;
	int N = 0;
	int net = Inet;
    
	returnSize(V, N, net, net); // Get size of the Network and number of Agents
	V+=1;
	// Define year
    int year = Iyear;
    ...
    for(int i1 = current_rank; i1 < total_runs; i1 += world_size) {
        ...
        if (Inet == 0){
            sprintf(fileOutput, "results/20132014/real/run_%d_%lf_%lf_%.1f_%.1f_real.csv", i, xv, biteRate, deltaT, deltaT1);
        }else if (Inet == 2){
            sprintf(fileOutput, "results/20132014/random/run_%d_%lf_%lf_%.1f_%.1f_random.csv", i, xv, biteRate, deltaT, deltaT1);
        }else if (Inet == 3){
            sprintf(fileOutput, "results/20132014/levy/run_%d_%lf_%lf_%.1f_%.1f_levy.csv", i, xv, biteRate, deltaT, deltaT1);
        }else if (Inet == 4){
            sprintf(fileOutput, "results/20132014/radial/run_%d_%lf_%lf_%.1f_%.1f_radial.csv", i, xv, biteRate, deltaT, deltaT1);
        }
		...
		if (file){
            ...
            Node  *K = initialize_nodes(N, V, net, Inet); // Initialize Nodes
            Agent *A = initialize_agents(N, V, net, net); // Initialize Agents
...
int main(int argc, char* argv[])
{
    ...
    if(argc != 4) {
        fprintf(stderr,"Error: missing parameters!\nUsage:\n./sim2013SD [total_iterations] [Iyear] [Inet]\n\n");
        return 1;
    }
    ...
    if(Inet < 0 || Inet > 4) {
        fprintf(stderr,"Error: Inet parameter must be 0, 1, 2 or 3!\nUsage:\n./sim2013SD [total_iterations] [Iyear] [Inet]\n\n");
        return 1;
    }
    ...
    perform(total_iterations, current_rank, world_size, Iyear, Inet);
